 ...
 *
 * This is a FORMidable data handler that can also handle M:M relations.
 *
...
class tx_seminars extends tx_ameosformidabledhdb {
	/**
	 * List of relations that are M:M relations as key/mm-table pairs
	 */
	var $mmFields = array(
	 ...
	 * For fields that have a m:m table defined in $this->mmFields, a real m:m
			...
			if ($this->_isSubmitted()) {
				...
				while (list($elementname, $value) = each($aFormData)) {
					if (array_key_exists($elementname, $this->oForm->aORenderlets)) {
						...
						// Do we have a field that should create a M:M relation?
				...
				if (!empty($tablename) && !empty($keyname)) {
					...
					if ($editEntry) {
						...
						// create all the M:M records,
						// but only if the main record has been created
