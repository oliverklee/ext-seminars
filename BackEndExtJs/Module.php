...
class tx_seminars_BackEndExtJs_Module extends t3lib_SCbase {
	/**
	 * path to the cached language labels
	 *
	 * @var string
	 */
	const LANGUAGE_LABELS_CACHE_PATH = 'typo3temp/tx_seminars/BackEndExtJs/';
	...
	public function main() {
		$this->getPageRenderer()->setCharSet('utf-8');
		...
		$this->getPageRenderer()->addJsFile(
			t3lib_div::getIndpEnv('TYPO3_SITE_PATH') . 't3lib/js/extjs/ux/flashmessages.js',
			'text/javascript',
			FALSE
		);
	...
	private function addLanguageLabelsFile() {
		...
		$languageCacheFile = self::LANGUAGE_LABELS_CACHE_PATH .
			'seminars_' . $language . '.js';
		$languageCacheFileAbsolute = PATH_site . $languageCacheFile;
	 ...
	 * and returns them as in array.
	 *
	 * @param string $language the key of the language to return the labels for
	 *
	 * @return array all language labels from the files listed in
	 *               $this->locallangFiles
	 */
	private function getLanguageLabels($language) {
		...
		foreach (self::$locallangFiles as $file) {
			$labelsInAllLanguages
				= t3lib_div::readLLfile($file, $language, 'utf-8');
...
if (defined('TYPO3_MODE') && $GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/seminars/BackEndExtJs/Module.php']) {
	include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/seminars/BackEndExtJs/Module.php']);
}
?>